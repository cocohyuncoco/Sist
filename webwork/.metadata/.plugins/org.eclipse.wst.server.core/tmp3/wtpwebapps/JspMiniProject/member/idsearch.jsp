...
<%
	...
	if(key==null){
		...
		<form action="idsearch.jsp" method="post" class="form-inline">
			...
			<table class="table table-bordered">
				<tr height="100">
					<td>
						<img alt="" src="../image2/f18.png" width="70" align="left">
						<br>
						
						<b>아이디를 입력해주세요</b>
						<br>
						
						<input type="text" name="id" class="form-control" style="width:200px;" required="required">
						<br>
	...
	<%}else{
		...
		<%}else{%>
			<h3 class="alert alert-success"><%=id %>는 사용가능한 아이디 입니다</h3>
			<button type="button" class="btn btn-waring btn-sm" id="btnuse" style="margin-left:200px;" myid="<%=id%>" onclick="goBack('<%=id%>')">사용하기</button>
			<button type="button" class="btn btn-danger btn-sm" onclick="location.href='idsearch.jsp'">다시하기</button>
		<%}
	}
%>
<script type="text/javascript">
